...
# 实时音频设置
chunk_length_sec = 2.0  # 每次处理 2.0 秒的音频
input_sample_rate = 44100  # 麦克风的采样率
model_sample_rate = 16000  # 模型的采样率
chunk = int(input_sample_rate * chunk_length_sec)  # 每次从麦克风读取的帧数
overlap = int(chunk * 0.5)  # 50%的重叠
channels = 1  # 单声道
...
def process_audio_chunk(audio_chunk):
    # 重采样到模型的采样率
    audio_chunk_resampled = torchaudio.transforms.Resample(orig_freq=input_sample_rate, new_freq=model_sample_rate)(torch.tensor(audio_chunk))

    # 处理音频
    inputs = processor(audio_chunk_resampled.numpy(), sampling_rate=model_sample_rate, return_tensors="pt", padding=True)
